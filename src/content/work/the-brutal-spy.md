---
title: The Brutal Spy
slug: the-brutal-spy
summary: A top-down action and stealth game for iPhone and iPad that I worked on early in my game-development career.
yearStart: 2012
status: Released mobile game
role: Game programmer
disciplines: [Games, Real-time systems]
visibility: public
featuredOrder: 0
recordType: evidence-note
era: programmer
domains: [games]
careerOrder: 10
relationships: [alphaman, merkur-magie]
evidenceStatus: public-corroborated
publicClaims:
  - The title and place in the career chronology are described in Pranshul's first-person public career narrative.
engagementPath: speaking-writing
storyLabel: SHIPPED GAME
storyHeading: A top-down stealth shooter built for early mobile hardware.
heroMedia:
  src: /media/work/the-brutal-spy/trailer-poster.webp
  label: The Brutal Spy · A combat encounter viewed from above, with touchscreen controls.
  alt: The Brutal Spy top-down stealth gameplay from the official launch trailer
  width: 1280
  height: 720
sources:
  - label: Official The Brutal Spy launch trailer
    url: https://www.youtube.com/watch?v=dB1g0Z5u3QU
    type: official-source
  - label: The Brutal Spy release listing — TouchArcade
    url: https://toucharcade.com/2012/01/25/coming-tonight-baseball-superstars-2012-hank-hazard-one-epic-game-paper-monsters-and-more/
    type: press
  - label: The Brutal Spy game details — Metacritic
    url: https://www.metacritic.com/game/the-brutal-spy/
    type: press
  - label: From Pixels to Metaverse
    url: https://2600th.substack.com/p/from-pixels-to-metaverse-my-wild
    type: authored-post
seo:
  title: The Brutal Spy — Early game work by Pranshul Chandhok
  description: The Brutal Spy marks an early point in Pranshul Chandhok's path from game programming to immersive systems, design technology, and applied AI.
  socialImage: /media/work/the-brutal-spy/trailer-poster.webp
---

Released for iPhone and iPad in January 2012, The Brutal Spy was a top-down action-adventure and stealth shooter. Players controlled Razor, a field operative, through close-quarters combat and encounters with armed enemies using touchscreen controls.

I worked on The Brutal Spy as a game programmer. It was one of my early experiences of developing a game for release on mobile hardware. The small screen and touchscreen controls made responsiveness especially important, while the action had to remain readable during combat.
